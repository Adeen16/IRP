...
public class TransactionDAOImpl implements TransactionDAO {
    ...
    public List<Transaction> findByAccountNumber(String accountNumber) {
        ...
        String sql = "SELECT transaction_id, account_number, type, amount, created_at, performed_by FROM \"transaction\" WHERE account_number = ? ORDER BY created_at DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<Transaction> findAll() {
        List<Transaction> list = new ArrayList<>();
        String sql = "SELECT transaction_id, account_number, type, amount, created_at, performed_by FROM \"transaction\" ORDER BY created_at DESC";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    public List<Transaction> findByAccountNumber(String accountNumber, int limit) {
        List<Transaction> list = new ArrayList<>();
        String sql = "SELECT transaction_id, account_number, type, amount, created_at, performed_by FROM \"transaction\" WHERE account_number = ? ORDER BY created_at DESC LIMIT ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, accountNumber);
            stmt.setInt(2, limit);
    ...
    public List<Transaction> findByAccountNumberAndDateRange(String accountNumber, LocalDate start, LocalDate end) {
        List<Transaction> list = new ArrayList<>();
        String sql = "SELECT transaction_id, account_number, type, amount, created_at, performed_by FROM \"transaction\" WHERE account_number = ? AND DATE(created_at) BETWEEN ? AND ? ORDER BY created_at DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, accountNumber);
    ...
    public List<Transaction> findByDateRange(LocalDate start, LocalDate end) {
        List<Transaction> list = new ArrayList<>();
        String sql = "SELECT transaction_id, account_number, type, amount, created_at, performed_by FROM \"transaction\" WHERE DATE(created_at) BETWEEN ? AND ? ORDER BY created_at DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private Transaction mapRowToTransaction(ResultSet rs) throws SQLException {
        ...
        t.setPerformedBy(rs.getInt("performed_by"));
        return t;
    }
}
